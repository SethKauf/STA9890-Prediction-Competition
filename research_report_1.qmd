...
To check for the unbiased property in the DGP, we can look towards Endogeneity, or that the errors are not affected by the input X variables: E[ϵ│X]=0. Looking at the input X variables:
...
OLS seeks to minimize some predictor variable β ̂ to minimize the error function. To do this, we look at the error term that OLS is trying to minimize, then substitute it in to find a proper value for β ̂:
RSS=∑_(i=1)^n▒ϵ_i^2 =|(|ϵ|)|^2
Since y=Xβ+ϵ we can rearrange the terms so ϵ=y-Xβ, expand the terms, then derive with respect to β:
RSS=|(|y-X|)|^2=(y-Xβ)^T (y-Xβ)
∂/∂β [(y-Xβ)^T (y-Xβ)]=∂/∂β [y^T y-2β^T X^T y+β^T X^T Xβ]
The y^T y term is a constant while 〖∂/∂β[β〗^T]=1 so we get the following and set it equal to 0 to minimize it:
-2X^T y+2X^T Xβ=0
〖2X〗^T Xβ=2X^T y
β=(X^T y)/(X^T X)=(X^T X)^(-1) X^T y
	OLS Gradient
Gradient Descent refers to the process of iteratively fitting models to the gradient of a function is the vector field whose value at point p gives the direction and rate of the fastest increase . In OLS terms, this translates to   where   is the loss function on the gradient. Gradient Descent keeps going until the parameter and objective each converge, or:   and  .
The setup is similar to the closed-form derivation from the previous step, it turns into:
β^((k+1))=β^k-c∇L|_(β=β^k )=β^k-c(-2X^T y+2X^T Xβ)=β^k-2c(X^T Xβ^k-X^T y)
...
β^((k+1))=β^k-c∇L|_(β=β^k )-wβ^k
Plugging this into the gradient derivative from the previous section, we get to this step:
∇L=-2X^T y+2X^T Xβ^k-2wβ^T β
β^((k+1))=β^k-c(2X^T y+2X^T Xβ^k-2wβ)
β^((k+1))=β^k-2c(X^T y+X^T Xβ^k-wI)
