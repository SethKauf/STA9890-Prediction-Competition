...
# y-target
table(data$readmit_at_all)


features <- setdiff(names(df), c("readmit_at_all", "readmit_early", "patient_nbr"))
nb_formula <- as.formula(paste("readmit_at_all ~", paste(features, collapse = "+")))

nb_model <- naiveBayes(nb_formula, data = train_df)
